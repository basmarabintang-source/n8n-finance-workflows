...
```
9 invoices worth 1.623.220.000 IDR came in.

Posted with no human touch: 1 (8.760.000 IDR)
Waiting on an approver:     2 (1.500.400.000 IDR)
Held in the exception queue: 6 (114.060.000 IDR)

Early-payment discounts worth 175.200 IDR are available if the schedule below is followed.
...
- [DUPLICATE]      Cahaya Teknik Presisi 8899   -> ap_supervisor - Already accepted 2026-08-29 as posted. Paying again would double-pay.
- [PRICE_VARIANCE] Cahaya Teknik Presisi INV-08 -> procurement   - Billed 20.160.000 vs expected 18.900.000. Over tolerance by 882.000.
```
...
| file | what |
|---|---|
| `ap-3way-match.json` | the pipeline, 26 nodes |
| `ap-reset-ledger.json` | empties the ledger and restores the one seeded payment, so the demo can be re-run from clean |
...
8. **Price** — line total vs PO price, within 2% or 50.000 IDR, whichever is larger. The
